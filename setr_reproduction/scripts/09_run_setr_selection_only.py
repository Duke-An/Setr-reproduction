"""Run Qwen3-14B SetR-Selection only on Dify Hybrid Top-20 candidates.

This script implements SETR reproduction step 6:
prompt-level set selection without explicit reasoning.
...
DEFAULT_PROMPT = PROJECT_ROOT / "setr_reproduction" / "prompts" / "setr_selection_only.txt"
...
def parse_selection(text: str, candidate_count: int, max_selected: int) -> Tuple[List[int], bool, str]:
    obj = extract_json_object(text)
    if obj and isinstance(obj.get("selected"), list):
        selected = normalize_selection(obj["selected"], candidate_count, max_selected)
        if selected:
            return selected, True, "json"

    numbers = re.findall(r"\b\d+\b", text)
    if numbers:
        selected = normalize_selection(numbers, candidate_count, max_selected)
        if selected:
            return selected, False, "regex"

    return list(range(1, min(candidate_count, max_selected) + 1)), False, "fallback_original_topk"
...
def select_one_row(
    ...
    try:
        ...
        selected_numbers, parse_success, parse_method = parse_selection(
            raw_content,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Run Qwen3-14B SetR-Selection only.")
...
def main() -> None:
    ...
    output_path = output_dir / "setr_selection_only_hybrid_500.jsonl"
    summary_json_path = output_dir / "setr_selection_only_hybrid_500_summary.json"
    summary_csv_path = output_dir / "setr_selection_only_hybrid_500_summary.csv"
    raw_output_path = raw_output_dir / "setr_selection_only_outputs_hybrid_500.jsonl"
